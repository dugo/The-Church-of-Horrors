...
def view_for_entry(request,entry):
    ...
    if not request.user.is_superuser and not request.user.get_profile().is_editor and not entry.published and (not request.user.is_authenticated() or entry.author_id <> request.user.id):
        return HttpResponseNotFound()
